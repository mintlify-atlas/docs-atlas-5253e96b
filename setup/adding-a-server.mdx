---
title: "Add and Configure a Server in ServerPilot"
sidebarTitle: "Adding a Server"
description: "Add your Minecraft server folder to ServerPilot and configure name, JAR, Java path, RAM, JVM arguments, and Playit.gg settings."
---
...
<ParamField path="autoStartPlayit" type="boolean" default="true">
  When `true`, ServerPilot spawns the Playit process when this server starts and terminates it when the server stops. The effective path used is `playitPath` (this server) falling back to the global `playitPath` (Settings). If neither is set, auto-start is skipped and a warning is logged to the console.
</ParamField>
