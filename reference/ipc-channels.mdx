...
Sends the `stop` command to the server's stdin and starts a 15-second force-kill timer. If the process does not exit within 15 seconds, it is terminated via `taskkill.exe /T /F`.
