---
title: "ServerPilot Troubleshooting Guide"
...
When the server stops (or is force-killed), ServerPilot sends a `kill()` signal to the Playit process it owns, with a 3-second fallback to `taskkill.exe /T /F` if it doesn't exit cleanly.
